...
#step 1
forest, xgb , gb, clf, xgb_clf, gb_clf = runIt('60d','15m',1,60)
...
def credit_swish(num_minutes, STOCK1):
    CS1 = stock_setup(STOCK1)

    for xx in range(num_minutes):
        #selling back
        final_t1 = flow2go_predictor(STOCK1,1, 60, CS1).fillna(0)
        try:
        ...
        if jj1 >=.6:
            ...
            place_limit_order(c,'sell',1,STOCK1,price*1.08)
        else:
            print('no dinero')

...
forest, xgb , gb, clf, xgb_clf, gb_clf = runIt('60d','5m',.001,15)
...
def printer(STOCK1,CS1,xx, bought):   
    ...
    x2 = x2/np.average(avg_probXGB[-20:])
    print('')
    print('')
    ...
    x3 = x3/np.average(avg_probGB[-20:])
    print('')
    print('')

    print('RF is '+str(x1-1)+'% off the average today')
    print('GB is '+str(x3-1)+'% off the average today')
    print('XGB is '+str(x2-1)+'% off the average today')
    print('')
    print('')
    if xx==0:
        bought1 = 0
...
#credit_swish(300,'FUBO', 'NGL','RIG','CCO','GCI')
#credit_swish(300,'ICD', 'SUP','RYAM','NINE','NR')
credit_swish(300,'PEI', 'USX','TISI','OIS','DBD')
